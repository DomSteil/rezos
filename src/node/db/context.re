...
/** Rezos - Versioned (key x value) store (over Irmin) */

open Logging.Db

module IrminPath = Irmin.Path.String_list;

module MBytesContent = {
  module Tc_S0 =
    (val Tc.biject Tc.cstruct Cstruct.to_bigarray Cstruct.of_bigarray)
  include Tc_S0
    module Path = Irmin.Path.String_list
    let merge = {
    let fn = Irmin.Merge.(option (module Tc_S0) (default (module Tc_S0)))
    fun _path => fn
  };
};
...
let commit key ~time ~message context =
  let task = Irmin.Task.create ~date:(Time.to_seconds time) ~owner:"Rezos" in
