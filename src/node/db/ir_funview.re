...
module type NODE = {
  ...
  let empty: unit => t;
  let is_empty: t => bool Lwt.t;

  let read: t => node option Lwt.t;

  let read_contents: t => Path.step => contents option Lwt.t;

  let with_contents: t => Path.step => contents option => t option Lwt.t;
  /* Return [true] iff the contents has actually changed. Used for
     invalidating the view cache if needed. */

  let read_succ: node => Path.step => t option;

  let with_succ: t => Path.step => t option => t option Lwt.t;
  /* Return [true] iff the successors has actually changes. Used for
     invalidating the view cache if needed. */

  let steps: t => Path.step list Lwt.t;
