...
let create_scheduler limits =
  let max_upload_speed =
    map_option limits.max_upload_speed ~f:(( * ) 1024);
...
let create_connection_pool config limits meta_cfg msg_cfg io_sched =
  ...
  in
  let pool =
    P2p_connection_pool.create pool_cfg meta_cfg msg_cfg io_sched;
  pool
...
module Real = {
  ...
  let create ~config ~limits meta_cfg msg_cfg =
    let io_sched = create_scheduler limits;
    create_connection_pool
      config limits meta_cfg msg_cfg io_sched >>= fun pool =>
    let discoverer = may_create_discovery_worker config pool ;
    let maintenance = create_maintenance_worker limits pool discoverer ;
  ...
  let rec recv_any net () =
    let pipes =
      P2p_connection_pool.Connection.fold
        ...
        (P2p_connection_pool.is_readable conn >>= fun
    ...
    | None => recv_any net ()
    | Some conn =>
        P2p_connection_pool.read conn >>= fun
        | Ok msg =>
            lwt_debug "message read from %a"
              Connection_info.pp
              (P2p_connection_pool.Connection.info conn) >>= fun () =>
  ...
  let send _net conn m =
    P2p_connection_pool.write conn m >>= fun
    | Ok () =>
