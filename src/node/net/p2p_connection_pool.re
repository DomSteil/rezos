  ...
  let encoding =
    ...
    union ~tag_size:`Uint8 [
      ...
      case ~tag:7 (branch_encoding "accepting_request"
        ...
        (fun Accepting_request (p, id_p, g) =>
      ...
      case ~tag:8 (branch_encoding "rejecting_request"
                     (obj3
                        ...
                        (req "id_point" Id_point.encoding)
                        (req "peer_id" Peer_id.encoding)))
        (fun Rejecting_request (p, id_p, g) =>
           Some (p, id_p, g) | _ => None)
...
module GcPointSet = Utils.Bounded(
...
let gc_peer_ids ({ meta_config = { score } ;
  ...
  | None => ()
  | Some (_, target) =>
      let table = GcPeer_idSet.create target;
      Peer_id.Table.iter (fun peer_id peer_info =>
          let created = Peer_info.created peer_info;
          let score = score @@ Peer_info.metadata peer_info;
...
let register_peer pool peer_id =
  ...
  | exception Not_found =>
      ...
      let peer = Peer_info.create peer_id ~metadata:pool.meta_config.initial;
    ...
    let candidates =
      fold pool ~init:[] ~f:begin fun _peer conn acc =>
        ...
        | Some excluded_conn
          when P2p_connection.equal conn.conn excluded_conn.conn => acc
        | Some _ | None =>
            let ci = P2p_connection.info conn.conn;
...
let compare_known_point_info p1 p2 =
  ...
  and disconnected2 = Point_info.State.is_disconnected p2;
