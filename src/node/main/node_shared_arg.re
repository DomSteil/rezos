...
let wrap
  ...
  let actual_data_dir =
    Utils.unopt ~default:Node_config_file.default_data_dir data_dir;

  let config_file =
    Utils.unopt ~default:(actual_data_dir // "config.json") config_file;
  ...
  let min_connections, expected_connections, max_connections =
    ...
    | None => None, None, None
    | Some x => Some (x/2), Some x, Some (3*x/2) ;
...
module Term = struct
  ...
  let log_output =
    let doc =
      "Log output. Either $(i,stdout), $(i,stderr), \
       $(i,syslog:<facility>) or a file path.";
  ...
  let config_file =
    let doc = "The main configuration file.";
    Arg.(value & opt (some string) None &
  ...
  let connections =
    let doc =
      "The number of running connections that we aim for.";
  ...
  let peer_table_size =
    let doc = "Maximum size of internal peer tables, \
               ...
               to-be-authenticated host:port couple.";
    Arg.(value & opt (some int) None &
  ...
  let peers =
    let doc =
      "A peer to bootstrap the network from. \
       Can be used several times to add several peers.";
  ...
  let closed =
    let doc =
      "Only accept connections from the configured bootstrap peers.";
  ...
  let rpc_tls =
    let doc =
      "Enable TLS for this RPC server \
       with the provided certificate and key.";
  ...
  let cors_origins =
    let doc =
      "CORS origin allowed by the RPC server \
       via Access-Control-Allow-Origin; may be used multiple times";
    Arg.(value & opt_all string [] &
  ...
  let cors_headers =
    let doc =
      "Header reported by Access-Control-Allow-Headers \
       reported during CORS preflighting; may be used multiple times";
    Arg.(value & opt_all string [] &
