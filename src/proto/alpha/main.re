...
let begin_application
  ...
  let level = header.shell.level;
  let fitness = pred_fitness;
  let timestamp = header.shell.timestamp;
  ...
  let mode = Application (header, miner);
...
let finalize_block { mode ; ctxt ; op_count } = match mode with
  ...
  | Application (block, miner) =>
      ...
      let priority = block.proto.priority;
      let level = Tezos_context.Raw_level.to_int32 level;
      let fitness = Tezos_context.Fitness.current ctxt;
      let commit_message =
        Format.asprintf
