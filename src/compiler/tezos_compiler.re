...
let create_file ?(perm = 0o644) name content =
  let open Unix;
  let fd = openfile name [O_TRUNC; O_CREAT; O_WRONLY] perm;
  ignore(write_substring fd content 0 (String.length content));
  close fd

let read_md5 file =
  let ic = open_in file;
  let md5 = input_line ic;
  close_in ic ;
  md5
...
module Protocol =
  ...
  let find_component dirname module_name =
    let name_lowercase = String.uncapitalize_ascii module_name;
    let implementation = dirname // name_lowercase ^ ".ml";
    let interface = implementation ^ "i";
    ...
    | true, false =>
        let implementation = Utils.read_file ~bin:false implementation;
        { name = module_name; interface = None; implementation };
    | _ =>
        let interface = Utils.read_file ~bin:false interface;
        let implementation = Utils.read_file ~bin:false implementation;
...
let compile_units
  ...
  let compile_unit update_needed unit =
    let basename = String.uncapitalize_ascii unit;
    let mli = source_dir // basename ^ ".mli";
    let cmi = build_dir // basename ^ ".cmi";
    let ml  = source_dir // basename ^ ".ml";
    let cmx = build_dir // basename ^ ".cmx";
    let target = build_dir  // basename;
    let update_needed =
      update_needed
...
let create_register_file client file hash packname modules =
  let unit = List.hd (List.rev modules);
  let environment_module = packname ^ ".Local_environment.Environment";
  let error_monad_module = environment_module ^ ".Error_monad";
  let context_module = environment_module ^ ".Context";
  let hash_module = environment_module ^ ".Hash";
  create_file file
    (Printf.sprintf
...
let main () =
  ...
  let client = !client and include_dirs = !include_dirs;
  let output, source_dir =
    match List.rev !anonymous with
    | [ output ; source_dir ] => output, source_dir
    | _ => Arg.usage args_spec usage_msg ; Pervasives.exit 1;
  if include_dirs <> [] && not client {
  ...
  let hash, units = Meta.of_file source_dir;
  ...
  let packed_objects =
    if keep_object {
      ...
      build_dir // packname ^ ".cmx";
  let ctxt = Printf.sprintf " (%s)" (Filename.basename output);
  let logname =
    if keep_object then
      try
        Scanf.sscanf
  ...
  let update_needed =
    ...
    || read_md5 (build_dir // ".tezos_compiler") <> md5;
  ...
  let local_environment_unit = "local_environment";
  let local_environment_ml = build_dir // local_environment_unit ^ ".ml";
  ...
  let register_unit =
    if client then
      ...
      build_dir // Format.asprintf "register_%s" packname;
  let register_file = register_unit ^ ".ml";
