...
let forge_request (meth, service) params input =
  let path, arg = Resto.forge_request service params input;
  meth, path, arg
...
let merge dirs1 dirs2 =
  let compare (meth1, _dir1) (meth2, _dir2) = compare meth1 meth2;
  let f (meth1, dir1) (_, dir2) = (meth1, RestoDirectory.merge dir1 dir2);
...
let register dirs (meth, service) handler =
  let init = RestoDirectory.empty;
  let f dir = RestoDirectory.register dir service handler;
...
let register_custom_lookup ?(meth = default_meth) ?descr dirs s f =
  let init = RestoDirectory.empty;
  let f dir = RestoDirectory.register_custom_lookup ?descr dir s f;
  replace_assoc ~init ~f meth dirs
...
let register_describe_directory_service dirs (meth, service) =
  let init = RestoDirectory.empty;
  let f dir = RestoDirectory.register_describe_directory_service dir service;
  replace_assoc ~init ~f meth dirs
